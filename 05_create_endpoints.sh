...
echo keystone user-role-add --user $admin_user_id --role $admin_role_id \
                       --tenant_id $admin_tenant_id


echo keystone user-role-add --user $nova_user_id --role $admin_role_id \
                       --tenant_id $service_tenant_id
echo keystone user-role-add --user $glance_user_id --role $admin_role_id \
                       --tenant_id $service_tenant_id

echo keystone user-role-add --user $admin_user_id --role $member_role_id \
                       --tenant_id $admin_tenant_id
...
echo keystone service-create --name nova --type compute --description \
    'OpenStack Compute Service'
echo keystone service-create --name volume --type volume --description \
    'OpenStack Volume Service'
echo keystone service-create --name glance --type image --description \
    'OpenStack Image Service'
echo keystone service-create --name keystone --type identity --description \
    'OpenStack Identity Service'
echo keystone service-create --name ec2 --type ec2 --description \
    'EC2 Service'
...
echo keystone endpoint-create \
    ...
    --publicurl "http://:5000/v2.0" \
    --adminurl "http://:35357/v2.0" \
    --internalurl "http://:5000/v2.0"
